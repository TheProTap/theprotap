...
import type { DNSRecord, DNSRecordType, DNSTemplate } from "@/lib/types/dns"
...
export default function EditTemplatePage({ params }: EditTemplatePageProps) {
  ...
  useEffect(() => {
    const fetchTemplate = async () => {
      try {
        ...
        setRecords(templateData.records)
      } catch (error) {
        toast({
          title: "Error",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await dnsTemplateService.updateTemplate(params.id, {
        name,
        description,
        records,
      })
      ...
      toast({
        title: "Success",
        description: "DNS template updated successfully",
      })

      router.push(`/dns-templates/${params.id}`)
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to update DNS template",
        variant: "destructive",
      })
    } finally {
  ...
  return (
    <div className="container mx-auto py-10">
      ...
      <h1 className="text-3xl font-bold mb-6">Edit DNS Template</h1>
